...
# ── Accent color helpers (JSON file, no GSchema key needed) ─────────────
import json as _json

def _accent_config_path():
    from gi.repository import GLib
    import os
    return os.path.join(GLib.get_user_config_dir(), "zeroneai_accent.json")

def _load_accent_color_file():
    try:
        with open(_accent_config_path()) as f:
            return _json.load(f).get("color", "#5D5CDE")
    except Exception:
        return "#5D5CDE"

def _save_accent_color_file(color: str):
    try:
        with open(_accent_config_path(), "w") as f:
            _json.dump({"color": color}, f)
    except Exception:
        pass
...
_SETTINGS_CSS = """
/* Compact rows */
adwactionrow > box > box > label { font-size: 13px; }
adwactionrow > box > box > .subtitle { font-size: 11px; opacity: 0.7; }
adwexpanderrow > box > box > label { font-size: 13px; }
adwexpanderrow > box > box > .subtitle { font-size: 11px; opacity: 0.7; }
row.activatable, row { min-height: 42px; }

/* Settings tab buttons - underline style like HTML */
.settings-tab-btn {
    border-radius: 0px;
...
.settings-tab-btn:checked {
    border-bottom-color: @accent_color;
    color: @accent_color;
}
.settings-tab-btn:not(:checked) { opacity: 0.65; }
...
/* Settings group listbox - full width, rounded */
.settings-group list,
.settings-group listview {
    border-radius: 8px;
}
...
_settings_css_installed = False
def _ensure_settings_css():
    global _settings_css_installed
    if not _settings_css_installed:
        provider = Gtk.CssProvider()
        provider.load_from_string(_SETTINGS_CSS)
        Gtk.StyleContext.add_provider_for_display(
            Gdk.Display.get_default(), provider,
            Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION - 1)
        _settings_css_installed = True


def _make_settings_page():
    """
    Full-width scrollable settings page (no Adw.Clamp centering).
    Returns (scrolled_window, content_box).
    Append Adw.PreferencesGroup widgets to content_box.
    """
    scroll = Gtk.ScrolledWindow(vexpand=True, hexpand=True)
    scroll.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)

    # spacing=16 puts 16px gap between each PreferencesGroup
...
class Settings(Gtk.Box):
    def __init__(self,app, controller: ZeroneController,headless=False, startup_page=None, popup=False, on_close=None, *args, **kwargs):
        super().__init__(orientation=Gtk.Orientation.VERTICAL, *args, **kwargs)
        _ensure_settings_css()
        self.app = app
        ...
        self._window = None  # will be set when presented as a window
        self.downloading = {}
        self.model_threads = {}
        self.slider_labels = {}
        ...
        self.model_threads = {}
        # Load custom prompts
        self.custom_prompts = self.controller.zeroneai_settings.custom_prompts 
        self.prompts_settings = self.controller.zeroneai_settings.prompts_settings
        ...
        # Page building — use full-width scrolled boxes instead of Adw.PreferencesPage
        self._llm_scroll,      self.LLMPage       = _make_settings_page()
        self._prompts_scroll,  self.PromptsPage    = _make_settings_page()
        self._tools_scroll,    self.ToolsPage      = _make_settings_page()
        self._memory_scroll,   self.MemoryPage     = _make_settings_page()
        self._general_scroll,  self.general_page   = _make_settings_page()
        ...
        help.connect("clicked", lambda button : Popen(get_spawn_command() + ["xdg-open", "https://github.com/b2-3c/ZeroneAI/wiki/User-guide-to-the-available-LLMs"]))
        self.LLM.set_header_suffix(help)
        
        # Add LLMs
        self.LLMPage.append(self.LLM)
        ...
        # Add LLMs
        self.MemoryPage.append(self.SECONDARY_LLM)
        group = Gtk.CheckButton()
        ...
        self.general_page.append(self.Voicegroup)
        ...
        self.PromptsPage.append(self.prompt)
        self.prompts_rows = []
        ...
        self.general_page.append(self.interface)
        ...
        lang_row = Adw.ComboRow(title=_("Language"), subtitle=_("Application interface language (restart required)"))
        lang_opts = tuple(languages)
        current_lang = self.settings.get_string("app-language")
        lang_helper = ComboRowHelper(lang_row, lang_opts, current_lang)
        def on_lang_changed(helper, value):
            self.settings.set_string("app-language", value)
        lang_helper.connect("changed", on_lang_changed)
        self.interface.add(lang_row)

        row = Adw.ActionRow(title=_("Interface Size"), subtitle=_("Adjust the size of the interface"))
        spin = Adw.SpinRow(adjustment=Gtk.Adjustment(lower=100, upper=250, value=self.controller.zeroneai_settings.zoom, page_increment=20, step_increment=10))
        row.add_suffix(spin)
        def update_zoom(x,y):
        ...
        entry = Gtk.Entry(text=self.controller.zeroneai_settings.username, valign=Gtk.Align.CENTER)
        entry.connect("changed", lambda entry: self.settings.set_string("user-name", entry.get_text()))
        row.add_suffix(entry)
        self.settings.bind("offers", int_spin, 'value', Gio.SettingsBindFlags.DEFAULT)
        self.interface.add(row)
        # ── Accent Color Group ────────────────────────────────────
        accent_group = Adw.PreferencesGroup(title=_("Accent Color"))
        self.general_page.append(accent_group)

        accent_row = Adw.ActionRow(title=_("Theme Color"),
                                   subtitle=_("Choose the primary accent color for the app"))
        accent_group.add(accent_row)

        # 6 color swatches matching HTML design
        ACCENT_COLORS = [
            ("#5D5CDE", _("Purple")),
            ("#3B82F6", _("Blue")),
            ("#10B981", _("Green")),
        ...
        swatches_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,
                               spacing=8, valign=Gtk.Align.CENTER,
                               margin_end=12)
        current_color = _load_accent_color_file()

        self._accent_btns = []
        for hex_color, color_name in ACCENT_COLORS:
            btn = Gtk.Button(tooltip_text=color_name,
                             valign=Gtk.Align.CENTER,
                             css_classes=["circular", "flat"])
            btn.set_size_request(28, 28)

            # Swatch using a DrawingArea-like box with inline CSS
            swatch = Gtk.Box()
            swatch.set_size_request(22, 22)
            provider = Gtk.CssProvider()
            is_active = (hex_color == current_color)
            border = "3px solid white" if is_active else "2px solid transparent"
            provider.load_from_string(f"""
                box {{
                    background-color: {hex_color};
                    border-radius: 50%;
                    border: {border};
                    box-shadow: 0 0 0 {"2px " + hex_color if is_active else "0px transparent"};
            ...
            def on_color_click(b, btns=self._accent_btns, app=self.app):
                _save_accent_color_file(b._hex)
                if hasattr(app, '_apply_accent_color'):
                    app._apply_accent_color(b._hex)
                for other in btns:
                    active = (other._hex == b._hex)
                    border = "3px solid white" if active else "2px solid transparent"
                    shadow = f"0 0 0 2px {other._hex}" if active else "0 0 0 0px transparent"
                    other._provider.load_from_string(f"""
                        box {{
                            background-color: {other._hex};
                            border-radius: 50%;
                            border: {border};
                            box-shadow: {shadow};
                            min-width: 22px; min-height: 22px;
        ...
        accent_row.add_suffix(swatches_box)

        # Browser
        self.build_browser_settings()
        self.general_page.append(self.browser_group)
        # Neural Network Control
        self.neural_network = Adw.PreferencesGroup(title=_('Neural Network Control'))
        self.general_page.append(self.neural_network) 
        ...
        self.general_page.append(self.developer)
        ...
        home_btn.connect("clicked", self._on_back)
        crumb_box.append(home_btn)
        crumb_box.append(Gtk.Label(label="›", css_classes=["breadcrumb-sep"]))
        crumb_box.append(Gtk.Label(label=_("Settings"), css_classes=["breadcrumb-current"]))
        header_row.append(crumb_box)

        # Title in CENTER
        title_lbl = Gtk.Label(label=_("Settings"),
                              css_classes=["title"],
                              halign=Gtk.Align.CENTER,
                              hexpand=True)
        header_row.append(title_lbl)

        # Empty box on the RIGHT to balance layout
        balance = Gtk.Box()
        balance.set_size_request(crumb_box.get_preferred_size()[1].width, -1)
        header_row.append(balance)

        # ── Compact tab bar (like HTML .tab-button) ──────────────
        ...
        # Use scrolled windows (full-width, no clamp)
        page_defs = [
            (self._llm_scroll,     "llm",     _("LLM")),
            (self._prompts_scroll, "prompts", _("Prompts")),
            (self._tools_scroll,   "tools",   _("Tools")),
            (self._memory_scroll,  "memory",  _("Knowledge")),
            (self._general_scroll, "general", _("General")),
        ]

        group_btn = None
        for pref_page, name, title in page_defs:
            self._stack.add_named(pref_page, name)
            btn = Gtk.ToggleButton(label=title,
                                   active=(name == "llm"),
                                   css_classes=["flat", "settings-tab-btn"])
            if group_btn is None:
                group_btn = btn
            else:
                btn.set_group(group_btn)
            btn.connect("toggled", self._on_tab_btn, name)
            tab_box.append(btn)

        self.append(self._stack)

        # Navigate to startup_page if specified
        if startup_page is not None:
            pages = {"LLM": "llm", "Prompts": "prompts", "Memory": "memory", "General": "general"}
            if startup_page in pages:
                self._stack.set_visible_child_name(pages[startup_page])
                # sync button active state
                for btn in tab_box:
                    if isinstance(btn, Gtk.ToggleButton):
                        btn.set_active(btn.get_label() == pages[startup_page])
    ...
    def _on_tab_btn(self, btn, name):
        if btn.get_active():
            self._stack.set_visible_child_name(name)

    def present(self, parent=None):
        """Present as a standalone window (legacy popup mode)."""
        win = Adw.Window()
        win.set_default_size(800, 700)
        win.set_modal(True)
        if parent:
            win.set_transient_for(parent)
        elif hasattr(self.app, "win"):
            win.set_transient_for(self.app.win)
        win.set_child(self)
        self._window = win
        win.present()
        return win
    ...
    def connect(self, signal, callback, *args):
        """Route window signals to the internal window."""
        if signal == "close-request" and self._window:
            return self._window.connect(signal, callback, *args)
        return super().connect(signal, callback, *args)
    
    def build_tools_page(self):
        self.tools_group = Adw.PreferencesGroup(title=_("Tools"))
        self.ToolsPage.append(self.tools_group)
        self.tool_rows = []
    ...
    def refresh_tools_list(self):
        ...
        tools_settings = self.controller.zeroneai_settings.tools_settings_dict
        # Get all tools
    ...
    def build_mcp_settings(self):
        ...
        self.ToolsPage.append(self.mcp_group)
    ...
    def refresh_mcp_servers_list(self):
        ...
        self.controller.zeroneai_settings.mcp_servers_dict = servers
    ...
    def remove_mcp_server(self, btn, server, identifier):
        servers = self.controller.zeroneai_settings.mcp_servers_dict
    ...
    def build_rag_settings(self):
        ...
        self.MemoryPage.append(self.RAG)
    ...
    def show_flatpak_sandbox_notice(self, el=None):
        ...
        dialog.set_body(_("ZERONE AI does not have enough permissions to run commands on your system, please run the following command"))
        dialog.add_response("close", _("Understood"))
        dialog.set_default_response("close")
        dialog.set_extra_child(CopyBox("flatpak --user override --talk-name=org.freedesktop.Flatpak --filesystem=home com.zeroneai.app", "bash", parent = self))
...
# Alias for inline use - Settings now works as both a widget and a window
