...
class ChatHistory(Gtk.Box):
    ...
    def __init__(self, window, chat, chat_id):
        ...
        apply_css_to_widget(self.chat_scroll, """
            scrollbar { opacity: 0; transition: opacity 300ms; }
        ...
        self.offers = self.controller.zeroneai_settings.offers
        self.build_offers()
    ...
    def populate_chat(self):
        if not self.controller.zeroneai_settings.virtualization:
    ...
    def _build_buttons(self):
        ...
        self.chat_stop_button.set_visible(False)
        # stop button moved to send button in chat_tab — not added here anymore
        ...
        self.button_clear.set_visible(False)
        # button_clear moved to sidebar ChatRow — hide permanently here
        ...
        self.regenerate_message_button.set_visible(False)
        # regenerate moved to assistant message buttons — hide permanently here
        # self.chat_controls_entry_block.append(self.regenerate_message_button)

    def build_placeholder(self):
        tips = [
            {"title": _("Ask about a website"), "subtitle": _("Write #https://website.com in chat to ask information about a website"), "on_click": lambda : self.send_bot_response(Gtk.Button(label="#https://github.com/b2-3c/ZeroneAI\nWhat is ZERONE AI?"))},
            ...
            {"title": _("Mini Window"), "subtitle": _("Ask questions on the fly using the mini window mode"), "on_click": lambda : open_website("https://github.com/b2-3c/ZeroneAI/?tab=readme-ov-file#mini-window-mode")},
            {"title": _("Text to Speech"), "subtitle": _("ZERONE AI supports text-to-speech! Enable it in the settings"), "on_click": lambda : self.app.settings_action_paged("General")},
            {"title": _("Keyboard Shortcuts"), "subtitle": _("Control ZERONE AI using Keyboard Shortcuts"), "on_click": lambda : self.app.on_shortcuts_action()},
            {"title": _("Prompt Control"), "subtitle": _("ZERONE AI gives you 100% prompt control. Tune your prompts for your use."), "on_click": lambda : self.app.settings_action_paged("Prompts")},
            {"title": _("Thread Editing"), "subtitle": _("Check the programs and processes you run from ZERONE AI"), "on_click": lambda : self.app.thread_editing_action()},
            {"title": _("Programmable Prompts"), "subtitle": _("You can add dynamic prompts to ZERONE AI, with conditions and probabilities"), "on_click": lambda : open_website("https://github.com/b2-3c/ZeroneAI/wiki/Prompt-variables")},
        ...
        box = Gtk.Box(
            ...
            spacing=14, vexpand=True,
            margin_start=24, margin_end=24,
        )

        # Logo with soft glow background
        logo_box = Gtk.Box(halign=Gtk.Align.CENTER)
        apply_css_to_widget(logo_box, """
            box {
                border-radius: 50%;
                padding: 18px;
        ...
        application_logo.set_pixel_size(64)
        logo_box.append(application_logo)
        box.append(logo_box)

        title_label = Gtk.Label(label=_("How can I help you today?"), css_classes=["title-2"])
        box.append(title_label)

        subtitle = Gtk.Label(
            label=_("Ask me anything — I'm here to assist."),
        ...
        self.offers_entry_block_placeholder = Gtk.Box(
            orientation=Gtk.Orientation.VERTICAL,
            spacing=6, valign=Gtk.Align.END, halign=Gtk.Align.CENTER,
            margin_bottom=12,
        )
        self.offers_entry_block_placeholder.set_size_request(-1, 40 * self.controller.zeroneai_settings.offers)
    ...
    def _animated_append(self, widget):
        """Append a widget with a fade-in animation."""
        widget.set_opacity(0)
        self.chat_list_block.append(widget)
        def fade_in(step=0):
            if step <= 10:
                widget.set_opacity(step / 10)
                GLib.timeout_add(16, fade_in, step + 1)
        GLib.idle_add(fade_in)
    ...
    def add_message(self, user, message=None, id_message=0, editable=False):
        """Add a message to the chat and return the box (Claude.ai style)"""

        # ── Update lazy load tracking ──────────────────────────────────────
        if self.lazy_load_enabled and id_message >= self.lazy_loaded_end:
        ...
        if user == "User":
            ...
            # Buttons column (left side of bubble, hidden until hover)
            buttons_col = Gtk.Box(
                orientation=Gtk.Orientation.VERTICAL,
            ...
            if editable:
                apply_edit_stack, branch_button = self.build_edit_box(row, str(id_message), False)
                evk = Gtk.GestureClick.new()
                evk.set_name(str(id_message))
                evk.set_button(3)
                row.add_controller(evk)
                ev = Gtk.EventControllerMotion.new()
                ev.connect("enter", lambda x, y, data, bc=buttons_col, bb=branch_button: (bc.set_visible(True), bb.set_visible(True)))
                ev.connect("leave", lambda data, bc=buttons_col, bb=branch_button: (bc.set_visible(False), bb.set_visible(False)))
                row.add_controller(ev)
                branch_button.set_visible(False)

                # Stack: label (edit controls) shown on hover
                buttons_col.append(apply_edit_stack)
                evk.connect("pressed", self.edit_message, None, apply_edit_stack)

            row.append(buttons_col)

            # Bubble
            bubble = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, halign=Gtk.Align.END, hexpand=False)
            bubble.add_css_class("user-bubble")
            if message is not None:
                bubble.append(message)

            if editable:
                overlay = Gtk.Overlay(hexpand=False)
                overlay.set_child(bubble)
                branch_button.set_halign(Gtk.Align.END)
                branch_button.set_valign(Gtk.Align.END)
                branch_button.set_margin_end(4)
                branch_button.set_margin_bottom(4)
                overlay.add_overlay(branch_button)
                row.append(overlay)
                # Fix edit_message to find the bubble box correctly
                evk.connect_after("pressed", lambda g, n, x, y, b=bubble, aes=apply_edit_stack: self.edit_message(g, n, x, y, b, aes) if n == 3 else None)
            else:
                row.append(bubble)

            self._animated_append(row)
            return row

        # ══════════════════════════════════════════════════════════════════
        # ASSISTANT / DONE  →  name + text on the LEFT, no avatar icon
        # ══════════════════════════════════════════════════════════════════
        if user in ("Assistant", "Done"):
            row = Gtk.Box(
                orientation=Gtk.Orientation.HORIZONTAL,
                halign=Gtk.Align.FILL, hexpand=True,
            ...
            col = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, hexpand=True, spacing=2)

            name_label = Gtk.Label(
                label=self.controller.zeroneai_settings.current_profile,
                halign=Gtk.Align.START,
                css_classes=["caption", "heading", "success" if user == "Done" else "warning"],
            )
            ...
            if editable:
                apply_edit_stack, branch_button = self.build_edit_box(row, str(id_message), True)
                evk = Gtk.GestureClick.new()
                ...
                # Hover shows buttons row below message
                buttons_row = Gtk.Box(
                    orientation=Gtk.Orientation.HORIZONTAL,
                    halign=Gtk.Align.START,
                    spacing=0,
                ...
                buttons_row.set_visible(False)
                buttons_row.append(apply_edit_stack)

                ev = Gtk.EventControllerMotion.new()
                ev.connect("enter", lambda x, y, data, br=buttons_row, bb=branch_button: (br.set_visible(True), bb.set_visible(True)))
                ev.connect("leave", lambda data, br=buttons_row, bb=branch_button: (br.set_visible(False), bb.set_visible(False)))
                row.add_controller(ev)
        ...
        # ══════════════════════════════════════════════════════════════════
        # ERROR message  →  left border red strip
        # ══════════════════════════════════════════════════════════════════
        if user == "Error":
            box = Gtk.Box(
                css_classes=["card"],
                margin_top=6, margin_start=20, margin_bottom=6, margin_end=20,
                halign=Gtk.Align.FILL,
            )
            apply_css_to_widget(box, """
                box {
            ...
            """)
            err_icon = Gtk.Image.new_from_gicon(Gio.ThemedIcon(name="dialog-error-symbolic"))
            err_icon.set_properties(margin_end=8, css_classes=["error"])
            box.append(err_icon)
            ...
            box.append(inner)
            self._animated_append(box)
            return box

        # ══════════════════════════════════════════════════════════════════
        # FILE / FOLDER  →  compact bubble right
        # ══════════════════════════════════════════════════════════════════
        if user in ("File", "Folder"):
            row = Gtk.Box(
                orientation=Gtk.Orientation.HORIZONTAL,
                halign=Gtk.Align.FILL, hexpand=True,
            ...
            bubble.add_css_class("user-bubble")
            icon_name = "document-symbolic" if user == "File" else "folder-symbolic"
            ficon = Gtk.Image.new_from_gicon(Gio.ThemedIcon(name=icon_name))
            ficon.set_properties(margin_end=6, css_classes=["accent"])
            bubble.append(ficon)
            if message is not None:
                bubble.append(message)
            row.append(bubble)
        ...
        # ══════════════════════════════════════════════════════════════════
        # WARNING / DISCLAIMER  →  centered info strip
        # ══════════════════════════════════════════════════════════════════
        box = Gtk.Box(
            css_classes=["card"],
            margin_top=6, margin_start=20, margin_bottom=6, margin_end=20,
        ...
        self.messages_box.append(box)
        content_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, hexpand=True)
        ...
        if user == "WarningNoVirtual":
            ...
            box_warning = Gtk.Box(halign=Gtk.Align.CENTER, orientation=Gtk.Orientation.HORIZONTAL, css_classes=["warning", "heading"])
            ...
            icon.set_icon_size(Gtk.IconSize.LARGE)
            icon.set_properties(margin_top=10, margin_start=20, margin_bottom=10, margin_end=10)
            box_warning = Gtk.Box(halign=Gtk.Align.CENTER, orientation=Gtk.Orientation.HORIZONTAL, css_classes=["heading"])
            box_warning.append(icon)
            label = Gtk.Label(
    ...
    def build_edit_box(self, box, id, has_prompt: bool = True):
        ...
        if has_prompt:
            info_button = Gtk.Button(
                ...
                css_classes=["flat", "msg-action-btn"],
                valign=Gtk.Align.CENTER,
                halign=Gtk.Align.CENTER,
            )
            ...
            # Regenerate button — only for assistant messages (has_prompt=True)
            regenerate_button = Gtk.Button(
                icon_name="view-refresh-symbolic",
                css_classes=["flat", "msg-action-btn"],
                valign=Gtk.Align.CENTER,
                tooltip_text=_("Regenerate response"),
            )
            regenerate_button.connect("clicked", lambda btn: self.emit("regenerate-requested"))
            buttons_box.append(regenerate_button)

            # TTS button — read message aloud
            tts_button = Gtk.Button(
                icon_name="audio-speakers-symbolic",
                css_classes=["flat", "msg-action-btn"],
                valign=Gtk.Align.CENTER,
                tooltip_text=_("Read aloud"),
            )
            tts_button.connect("clicked", self._on_tts_message, int(id))
            buttons_box.append(tts_button)
    ...
    def apply_edit_message(self, gesture, box: Gtk.Box, apply_edit_stack: Gtk.Stack):
        ...
        # Navigate to content_box (new structure: box is the bubble/content box directly)
        content_box = box
        first = box.get_first_child()
        if first is not None and isinstance(first, Gtk.Overlay):
    ...
    def _on_tts_message(self, button, id):
        """Read message aloud — button becomes stop button while playing."""
        try:
            tts = self.controller.handlers.tts

            # If already playing (button is in stop mode) — stop it
            if getattr(button, '_tts_playing', False):
                tts.stop()
                return

            text = self.chat[id]["Message"]
            if not text:
                return
            ...
            button._tts_playing = True
            button.set_icon_name("media-playback-stop-symbolic")
            button.set_tooltip_text(_("Stop reading"))
            button.add_css_class("destructive-action")
            button.remove_css_class("msg-action-btn")

            # Hide the header mute button — we handle stop ourselves
            win = self.get_root()
            if win and hasattr(win, 'mute_tts_button'):
                GLib.idle_add(win.mute_tts_button.set_visible, False)
            ...
            def speak():
                try:
                    ...
                    print(f"TTS error: {e}")
                # Reset button back to play mode
                def reset():
                    button._tts_playing = False
                    button.set_icon_name("audio-speakers-symbolic")
                    button.set_tooltip_text(_("Read aloud"))
                    button.remove_css_class("destructive-action")
                    button.add_css_class("msg-action-btn")
                GLib.idle_add(reset)
            threading.Thread(target=speak, daemon=True).start()
        except Exception as e:
            print(f"TTS button error: {e}")
    ...
    def edit_message(
        ...
        # Find the message content box:
        # New structure: box is either the bubble directly, or a vertical box containing message widget
        # Try to find the last child that is the message widget
        content_box = box
        
        # If box has an overlay as first child (old structure), navigate through it
        first = box.get_first_child()
        if first is not None and isinstance(first, Gtk.Overlay):
            inner = first.get_child()
            if inner is not None:
        ...
        entry = MultilineEntry(not self.controller.zeroneai_settings.send_on_enter)
    ...
    def _wrap_message_box(self, user_type: str, content_box, id_message: int, editable: bool):
        """Wrap a content box in the message wrapper (Claude.ai style, matches add_message)"""
        ...
        if user_type == "User":
            ...
            spacer = Gtk.Box(hexpand=True)
            row.append(spacer)
            wrapper_box = Gtk.Box(halign=Gtk.Align.END, hexpand=False)
            wrapper_box.add_css_class("user-bubble")
            overlay = Gtk.Overlay(hexpand=False)
            overlay.set_child(wrapper_box)
            if editable:
                apply_edit_stack, branch_button = self.build_edit_box(row, str(id_message), False)
                evk = Gtk.GestureClick.new()
        ...
        # ── ASSISTANT ─────────────────────────────────────────────────────
        if user_type in ("Assistant", "Done"):
            row = Gtk.Box(
                orientation=Gtk.Orientation.HORIZONTAL,
                halign=Gtk.Align.FILL, hexpand=True,
                margin_top=8, margin_bottom=8,
            ...
            avatar_box = Gtk.Box(valign=Gtk.Align.START, halign=Gtk.Align.CENTER)
            apply_css_to_widget(avatar_box, """
                box {
            ...
            """)
            ai_icon = Gtk.Image.new_from_gicon(Gio.ThemedIcon(name="starred-symbolic"))
            ai_icon.set_icon_size(Gtk.IconSize.NORMAL)
            apply_css_to_widget(ai_icon, "image { color: white; }")
            avatar_box.append(ai_icon)
            row.append(avatar_box)
            col = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, hexpand=True, spacing=2)
            name_label = Gtk.Label(
                label=self.controller.zeroneai_settings.current_profile,
                halign=Gtk.Align.START,
                css_classes=["caption", "heading", "success" if user_type == "Done" else "warning"],
            )
            col.append(name_label)
            wrapper_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, hexpand=True)
            overlay = Gtk.Overlay(hexpand=True)
            overlay.set_child(wrapper_box)
            if editable:
                apply_edit_stack, branch_button = self.build_edit_box(row, str(id_message), True)
        ...
        if user_type in ("File", "Folder"):
            ...
            spacer = Gtk.Box(hexpand=True)
            row.append(spacer)
            wrapper_box = Gtk.Box(halign=Gtk.Align.END)
            wrapper_box.add_css_class("user-bubble")
            wrapper_box.append(content_box)
            row.append(wrapper_box)
            return row

        # ── Fallback ──────────────────────────────────────────────────────
        wrapper_box = Gtk.Box(
            css_classes=["card"],
            margin_top=6, margin_start=14, margin_bottom=6, margin_end=14,
            halign=Gtk.Align.FILL,
        )
        wrapper_box.append(content_box)
        return wrapper_box
    ...
    def populate_suggestions(self, suggestions):
        ...
        i = 0
        offers_count = self.controller.zeroneai_settings.offers
