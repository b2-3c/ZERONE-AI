...
class ChatTab(Gtk.Box):
    ...
    def _build_input_box(self):
        ...
        # Single card containing text + buttons
        self.input_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, halign=Gtk.Align.FILL)
        ...
        # Text entry — transparent, blends into the card
        self.input_panel = MultilineEntry(not self.controller.zeroneai_settings.send_on_enter)
        self.input_panel.set_on_image_pasted(self.image_pasted)
        self.input_panel.set_placeholder(_("Message ZeroneAI..."))
        self.input_panel.set_hexpand(True)
        self.input_panel.remove_css_class("card")
        self.input_panel.remove_css_class("frame")
        apply_css_to_widget(self.input_panel, "box { background: transparent; border: none; box-shadow: none; }")
        ...
        # Bottom toolbar
        toolbar = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0, valign=Gtk.Align.CENTER)

        self._build_quick_toggles()
        toolbar.append(self.quick_toggles)

        self.attach_button = Gtk.Button(
            css_classes=["flat", "circular"], icon_name="attach-symbolic", valign=Gtk.Align.CENTER,
        )
        ...
        self.screen_record_button = Gtk.Button(
            icon_name="media-record-symbolic", css_classes=["flat", "circular"],
        ...
        if self.model.supports_video_vision():
            self.screen_record_button.set_visible(True)
        toolbar.append(self.screen_record_button)

        toolbar.append(Gtk.Box(hexpand=True))  # spacer

        self.mic_button = Gtk.Button(
            css_classes=["flat", "circular"], icon_name="audio-input-microphone-symbolic",
            valign=Gtk.Align.CENTER, width_request=32, height_request=32,
        )
        self.mic_button.connect("clicked", self.start_recording)
        self.recording_button = self.mic_button
        toolbar.append(self.mic_button)

        self.send_button = Gtk.Button(
            css_classes=["suggested-action", "circular"], icon_name="go-next-symbolic",
            width_request=32, height_request=32, valign=Gtk.Align.CENTER,
        )
        toolbar.append(self.send_button)

        self.input_box.append(toolbar)
        outer.append(self.input_box)
    ...
    def tts_enabled(self) -> bool:
        ...
        return self.controller.zeroneai_settings.tts_enabled
    ...
    def switch_to_chat(self, chat_id: int):
        ...
        reverse_order = self.controller.zeroneai_settings.reverse_order
    ...
    def set_input_text(self, text: str):
        """Set text in the input panel (used by Library templates)."""
    ...
    def on_entry_activate(self, entry):
        ...
        # Block send if entry is empty and no attachment
        if (not text or text.isspace()) and self.attached_image_data is None:
            return False
        ...
        elif self.attached_image_data is not None:
            # Attachment only with no text
            if self.attached_image_data.endswith((".png", ".jpg", ".jpeg", ".webp")) or \
               self.attached_image_data.startswith("data:image/jpeg;base64,"):
                text = "```image\n" + self.attached_image_data + "\n```"
            elif self.attached_image_data.endswith((".mp4", ".mkv", ".webm", ".avi")):
    ...
    def send_button_start_spinner(self):
        """Switch send button to stop button during generation."""
        self.send_button.set_icon_name("media-playback-stop-symbolic")
        self.send_button.remove_css_class("suggested-action")
        self.send_button.add_css_class("destructive-action")
        # Reconnect to stop instead of send
        try:
            self.send_button.disconnect_by_func(self.on_entry_button_clicked)
        except Exception:
            pass
        self._send_btn_stop_handler = self.send_button.connect("clicked", lambda _: self._on_stop_requested(None))

    def remove_send_button_spinner(self):
        """Restore send button after generation."""
        self.send_button.set_icon_name("go-next-symbolic")
        self.send_button.remove_css_class("destructive-action")
        self.send_button.add_css_class("suggested-action")
        try:
            if hasattr(self, '_send_btn_stop_handler'):
                self.send_button.disconnect(self._send_btn_stop_handler)
                del self._send_btn_stop_handler
        except Exception:
            pass
        try:
            self.send_button.disconnect_by_func(self.on_entry_button_clicked)
    ...
    def _handle_generation_finished(self, data, stream_number_variable):
        ...
        if hasattr(self, "current_streaming_message") and self.current_streaming_message:
            ...
            def finalize_stream():
                ...
                if streaming_widget.state.get("has_terminal_command", False):
                    ...
                    parallel = self.controller.zeroneai_settings.parallel_tool_execution
        ...
        # Generate chat name
        if self.controller.zeroneai_settings.auto_generate_name and len(self.chat) == 2:
        ...
        if self.tts_enabled:
            ...
            if not(not message.strip() or message.isspace() or all(char == '\n' for char in message)):
                ...
                translator = None
                if self.controller.zeroneai_settings.translation_enabled:
                    translator = self.controller.handlers.translator          
                if self.controller.zeroneai_settings.avatar_enabled and self.controller.handlers.avatar is not None:
        ...
        def generate():
            try:
                name = self.window.secondary_model.generate_chat_name(
                    self.controller.zeroneai_settings.prompts["generate_name_prompt"],
    ...
    def generate_suggestions(self):
        """Create the suggestions and update the UI when finished"""
        ...
        def generate():
            try:
                suggestions = self.controller.handlers.secondary_llm.get_suggestions(
                    ...
                    self.controller.zeroneai_settings.offers,
