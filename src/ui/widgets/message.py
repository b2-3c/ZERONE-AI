...
class Message(Gtk.Box):
    ...
    def update_content(self, message: str, is_streaming: bool = False):
        ...
        chunks = get_message_chunks(message, allow_latex=self.controller.zeroneai_settings.display_latex)
    ...
    def _process_inline_chunks(self, chunk, box):
        ...
        for i, subchunk in enumerate(chunk.subchunks):
            if subchunk.type == "text":
                ...
                try:
                    font_size = int(5 + (self.controller.zeroneai_settings.zoom / 100 * 4))
    ...
    def _process_console_codeblock(self, chunk, box, state, restore):
        ...
        can_auto_run = (self.controller.zeroneai_settings.auto_run and not any(cmd in command for cmd in dangerous_commands) and self.parent_window.auto_run_times < self.controller.zeroneai_settings.max_run_times)
    ...
    def _run_tool_call_with_placeholder(self, tool, args, tool_uuid, state, restore, placeholder, chunk):
        ...
        def run_tool():
            try:
                ...
                if self.controller.zeroneai_settings.parallel_tool_execution or restore:
                    t.start()
            except Exception as e:
