...
class NyarchApiHandler(OpenAIHandler):
    key = "zeroneai"
    error_message = """Error calling ZERONE AI API. Please note that ZERONE AI API is **just for demo purposes.**\n\nTo know how to use a more reliable LLM [read our guide to llms](https://github.com/b2-3c/ZeroneAI/wiki/User-guide-to-the-available-LLMs). \n\nError: """

    def __init__(self, settings, path):
        super().__init__(settings, path)
        self.set_setting("endpoint", "https://llm.zeroneai.moe")
    ...
    def get_extra_settings(self) -> list:
        return self.build_extra_settings("ZERONE AI",False, True, False, False, False, None, None, False, False)

    def generate_text_stream(self, prompt: str, history: list[dict[str, str]] = [], system_prompt: list[str] = [], on_update: Callable[[str], Any] = lambda _: None, extra_args: list = []) -> str:
        if prompt.startswith("/chatname"):
            self.set_setting("endpoint", "https://llm.zeroneai.moe/small")
        elif prompt.startswith("```image") or  any(message.get("Message", "").startswith("```image") for message in history):
            self.set_setting("endpoint", "https://llm.zeroneai.moe/vision")
