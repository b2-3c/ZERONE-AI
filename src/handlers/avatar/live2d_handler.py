...
class Live2DHandler(AvatarHandler):
    ...
    def get_expression_manager(self):
        ...
        if len(li) == 0:
            return [
                ExtraSettings.ButtonSetting("no_entry", "You need to reload the model first", "Close and reopen the settings after changing a model in order to use the expression manager", lambda x: open_website("https://wiki.zeroneai.moe/nyarchasssistant",), "ZERONE AI Wiki")
            ]
    ...
    def install(self):
        ...
        subprocess.check_output(["wget", "-P", os.path.join(self.models_dir), "http://mirror.zeroneai.moe/Arch.tar.xz"])
