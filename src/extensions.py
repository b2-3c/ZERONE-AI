...
class ZeroneExtension(Handler):
    ...
    def preprocess_history(self, history: list, prompts : list) -> tuple[list, list]:
        """
        Called on the history before it is sent to the LLM. History is given in ZERONE AI format
        """
        return history, prompts

    def postprocess_history(self, history: list, bot_response: str) -> tuple[list, str]:
        """
        Called on the history after it is received from the LLM. History is given in ZERONE AI format
        """
...
class ExtensionLoader:
    ...
    def __init__(self, extension_dir, project_dir=None, pip_path="", extension_cache="", settings=None):
        ...
        self.extensions : list[ZeroneExtension] = []
        self.extensionsmap : dict[str, ZeroneExtension] = {}
        self.disabled_extensions : list[ZeroneExtension] = []
        self.codeblocks : dict[str, ZeroneExtension] = {}
        self.filemap : dict[str, str] = {}

    def get_extensions(self) -> list[ZeroneExtension]:
        return self.extensions

    def get_enabled_extensions(self) -> list[ZeroneExtension]:
    ...
    def load_extensions(self):
        ...
        for file in os.listdir(self.extension_dir):
            if file.endswith(".py"):
                try: 
                    spec = importlib.util.spec_from_file_location("zeroneai.name", os.path.join(self.extension_dir, file))
                    module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(module)

                    for class_name, class_obj in module.__dict__.items():
                        if isinstance(class_obj, type) and issubclass(class_obj, ZeroneExtension) and class_obj != ZeroneExtension:
    ...
    def remove_tools(self, tool_registry, extension: ZeroneExtension):
        for tool in extension.get_tools():
    ...
    def remove_extension(self, extension : ZeroneExtension | str):
        """
    ...
    def get_extension_by_id(self, id: str) -> ZeroneExtension | None:
        """
        ...
        Returns:
            ZeroneExtension | None: the extension or None if not found 
        """
        if id in self.extensionsmap:
            return self.extensionsmap[id]
        return None

    def enable(self, extension : ZeroneExtension | str):
        """
        Enable an extension
    ...
    def disable(self, extension : ZeroneExtension | str): 
        """
    ...
    def check_validity(self, extension : ZeroneExtension):
        """
        ...
        """
        if not hasattr(extension, "id") or not hasattr(extension, "name") or len(extension.id) > 50 or len(extension.name) > 50 or extension.id == ZeroneExtension.id or extension.name == ZeroneExtension.name:
