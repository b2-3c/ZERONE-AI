...
if is_flatpak():
    BASE_PATH = "/app/data"
    ZERONEAI_PATH = os.path.join(BASE_PATH, "live2d/web/zerone-ai.png")
else:
    BASE_PATH = "/usr/share/zeroneai/data"
    ZERONEAI_PATH = os.path.join(BASE_PATH, "live2d/web/zerone-ai.png") 
"""
Manage ZERONE AI Application, create handlers, check integrity, manage settings...
"""
...
class ZeroneController:
    ...
    def chat(self):
        ...
        if hasattr(self, 'chats') and hasattr(self, 'zeroneai_settings'):
            chat_id = self.zeroneai_settings.chat_id
    ...
    def ui_init(self):
        ...
        self.zeroneai_settings = ZeroneSettings()
        self.zeroneai_settings.load_settings(self.settings)
        self.load_chats(self.zeroneai_settings.chat_id)
        ...
        self.handlers.select_handlers(self.zeroneai_settings)
    ...
    def init_paths(self) -> None:
        ...
        self.zeroneai_dir = os.path.join(self.config_dir, DIR_NAME)
    ...
    def check_path_integrity(self):
        ...
        if not os.path.exists(self.zeroneai_dir):
            os.makedirs(self.zeroneai_dir, exist_ok=True)
        # Fix Pip environment
        ...
        # ZERONE AI profile image

        if not os.path.exists(self.config_dir + "/profiles/zerone-ai.png"):
            subprocess.run(["wget", "-O", self.config_dir + "/profiles/zerone-ai.png", "https://zeroneai.moe/acchan.png"])
    ...
    def update_settings(self, apply=True):
        """Update settings"""
        newsettings = ZeroneSettings()
        newsettings.load_settings(self.settings)
        reload = self.zeroneai_settings.compare_settings(newsettings)
        if apply:
            self.zeroneai_settings = newsettings
            for r in reload:
                self.reload(r)
        return reload
    ...
    def reload(self, reload_type: ReloadType):
        ...
        if reload_type == ReloadType.EXTENSIONS:
            ...
            self.zeroneai_settings.load_prompts()
            ...
            self.handlers.select_handlers(self.zeroneai_settings)
            if self.zeroneai_settings.use_secondary_language_model:
            ...
            self.handlers.select_handlers(self.zeroneai_settings)
            if self.zeroneai_settings.rag_on:
            ...
            self.handlers.select_handlers(self.zeroneai_settings)
            self.zeroneai_settings.save_prompts()
            self.zeroneai_settings.load_prompts()
    ...
    def get_enabled_tools(self) -> list:
        ...
        """
        enabled_tools = []
        tools_settings = self.zeroneai_settings.tools_settings_dict
        ...
        for tool in self.tools.get_all_tools():
            ...
            if tool.name == "search" and not self.zeroneai_settings.websearch_on:
                is_enabled = False
    ...
    def create_profile(self, profile_name, picture=None, settings={}, settings_groups=[]):
        ...
        """
        self.zeroneai_settings.profile_settings[profile_name] = {"picture": picture, "settings": settings, "settings_groups": settings_groups}
        self.settings.set_string("profiles", json.dumps(self.zeroneai_settings.profile_settings))
    ...
    def delete_profile(self, profile_name):
        ...
        if profile_name == "Assistant" or profile_name == self.settings.get_string("current-profile"):
            return
        del self.zeroneai_settings.profile_settings[profile_name]
        self.settings.set_string("profiles", json.dumps(self.zeroneai_settings.profile_settings))
    ...
    def update_current_profile(self):
        ...
        self.profile_settings = self.zeroneai_settings.profile_settings
    ...
    def import_profile(self, js):
        ...
        Args:
            json (): json to import
        """
        self.zeroneai_settings.profile_settings[js["name"]] = js
        if self.zeroneai_settings.profile_settings[js["name"]]["picture"] is not None:
            image_str = self.zeroneai_settings.profile_settings[js["name"]]["picture"]
            ...
            with open(img_path, "wb") as f:
                f.write(raw_data)
            self.zeroneai_settings.profile_settings[js["name"]]["picture"] = img_path
        ...
        for tool in tools:
            if tool.name == name:
                if tool in self.get_enabled_tools():
                    ...
                    return False
        if name == "tts_on":
            return self.zeroneai_settings.tts_enabled
        elif name == "virtualization_on":
            return self.zeroneai_settings.virtualization
        elif name == "auto_run":
            return self.zeroneai_settings.auto_run
        elif name == "websearch_on":
            return self.zeroneai_settings.websearch_on
        elif name == "rag_on":
            return self.zeroneai_settings.rag_on_documents
        elif name == "local_folder":
            return self.zeroneai_settings.rag_on
        elif name == "automatic_stt":
            return self.zeroneai_settings.automatic_stt
        elif name == "profile_name":
            return self.zeroneai_settings.current_profile
        elif name == "external_browser":
            return self.zeroneai_settings.external_browser
        elif name == "history":
    ...
    def get_history(
        ...
        history = []
        count = self.zeroneai_settings.memory
        ...
        for msg in msgs:
            ...
            if msg["User"] == "Console" and msg["Message"] == "None":
                continue
            if self.zeroneai_settings.remove_thinking:
    ...
    def get_memory_prompt(self, chat=None, chat_id=None):
        ...
        r = []
        if self.zeroneai_settings.memory_on:
        ...
        if (
            self.zeroneai_settings.rag_on_documents
            ...
            if len(documents) > 0:
                ...
                if existing_index.get_index_size() > self.zeroneai_settings.rag_limit: 
    ...
    def prepare_generation(self, chat_id=None):
        ...
        effective_chat_id = chat_id if chat_id is not None else self.zeroneai_settings.chat_id
        chat = self.get_chat_by_id(effective_chat_id)
        
        # Save profile for generation 
        self.chats[effective_chat_id]["profile"] = self.zeroneai_settings.current_profile

        # Append extensions prompts
        prompts = []
        formatter = PromptFormatter(replace_variables_dict(), self.get_variable)
        for prompt in self.zeroneai_settings.bot_prompts:
            prompts.append(formatter.format(prompt))

        # Inject active library character prompt if available
        if self.ui_controller is not None and hasattr(self.ui_controller.window, "get_active_character_prompt"):
            char_prompt = self.ui_controller.window.get_active_character_prompt()
            if char_prompt:
                prompts.insert(0, char_prompt)
...
class ZeroneSettings:
    ...
    def load_settings(self, settings):
        ...
        if len(self.profile_settings) == 0 or self.current_profile not in self.profile_settings:
            if is_flatpak():
                path = "/app/data/live2d/web/zerone-ai.png"
            else:
                path = None
    ...
    def compare_settings(self, new_settings) -> list[ReloadType]:
        """Find the difference between two ZeroneSettings

        Args:
            new_settings (ZeroneSettings): settings to compare   
...
class HandlersManager:
    ...
    def fix_handlers_integrity(self, zeroneai_settings: ZeroneSettings):
        """Select available handlers if not available handlers in settings

        Args:
            zeroneai_settings: ZERONE AI settings
        """
        if zeroneai_settings.language_model not in AVAILABLE_LLMS:
            zeroneai_settings.language_model = list(AVAILABLE_LLMS.keys())[0]
        if zeroneai_settings.secondary_language_model not in AVAILABLE_LLMS:
            zeroneai_settings.secondary_language_model = list(AVAILABLE_LLMS.keys())[0]
        if zeroneai_settings.embedding_model not in AVAILABLE_EMBEDDINGS:
            zeroneai_settings.embedding_model = list(AVAILABLE_EMBEDDINGS.keys())[0]
        if zeroneai_settings.memory_model not in AVAILABLE_MEMORIES:
            zeroneai_settings.memory_model = list(AVAILABLE_MEMORIES.keys())[0]
        if zeroneai_settings.rag_model not in AVAILABLE_RAGS:
            zeroneai_settings.rag_model = list(AVAILABLE_RAGS.keys())[0]
        if zeroneai_settings.tts_program not in AVAILABLE_TTS:
            zeroneai_settings.tts_program = list(AVAILABLE_TTS.keys())[0]
        if zeroneai_settings.stt_engine not in AVAILABLE_STT:
            zeroneai_settings.stt_engine = list(AVAILABLE_STT.keys())[0]
        if zeroneai_settings.websearch_model not in AVAILABLE_WEBSEARCH:
            zeroneai_settings.websearch_model = list(AVAILABLE_WEBSEARCH.keys())[0]
        if zeroneai_settings.avatar not in AVAILABLE_AVATARS:
            zeroneai_settings.avatar = list(AVAILABLE_AVATARS.keys())[0]
        if zeroneai_settings.translator not in AVAILABLE_TRANSLATORS:
            zeroneai_settings.translator = list(AVAILABLE_TRANSLATORS.keys())[0]
    
    def set_ui_controller(self, ui_controller):
        self.ui_controller = ui_controller
    
    def select_handlers(self, zeroneai_settings: ZeroneSettings):
        """Assign the selected handlers

        Args:
            zeroneai_settings: ZERONE AI settings 
        """
        self.fix_handlers_integrity(zeroneai_settings)
        # Get LLM 
        self.llm : LLMHandler = self.get_object(AVAILABLE_LLMS, zeroneai_settings.language_model)
        if zeroneai_settings.use_secondary_language_model:
            self.secondary_llm : LLMHandler = self.get_object(AVAILABLE_LLMS, zeroneai_settings.secondary_language_model, True)
        else:
            self.secondary_llm : LLMHandler = self.llm
        self.stt : STTHandler = self.get_object(AVAILABLE_STT, zeroneai_settings.stt_engine)
        self.tts : TTSHandler = self.get_object(AVAILABLE_TTS, zeroneai_settings.tts_program)
        self.embedding : EmbeddingHandler= self.get_object(AVAILABLE_EMBEDDINGS, zeroneai_settings.embedding_model)
        self.memory : MemoryHandler = self.get_object(AVAILABLE_MEMORIES, zeroneai_settings.memory_model)
        self.memory.set_memory_size(zeroneai_settings.memory)
        self.rag : RAGHandler = self.get_object(AVAILABLE_RAGS, zeroneai_settings.rag_model)
        self.websearch : WebSearchHandler = self.get_object(AVAILABLE_WEBSEARCH, zeroneai_settings.websearch_model)
        self.avatar : AvatarHandler = self.get_object(AVAILABLE_AVATARS, zeroneai_settings.avatar)
        self.translator : TranslatorHandler = self.get_object(AVAILABLE_TRANSLATORS, zeroneai_settings.translator)
        # Assign handlers 
    ...
    def get_constants_from_object(self, handler: Handler) -> dict[str, Any]:
        ...
        if issubclass(type(handler), TTSHandler):
            ...
            return AVAILABLE_LLMS
        elif issubclass(type(handler), ZeroneExtension):
            return self.extensionloader.extensionsmap
