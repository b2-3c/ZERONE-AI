...
from .constants import AVAILABLE_LLMS, AVAILABLE_TRANSLATORS, PROMPTS, AVAILABLE_TTS, AVAILABLE_STT, AVAILABLE_PROMPTS
import threading
import posixpath
import json 
import base64
...
from .controller import ZeroneController, ReloadType
from .ui_controller import UIController
from .extensions import ExtensionLoader
from .ui.library import LibraryWindow, LibraryManager, TemplatePickerDialog
...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        # Init controller
        self.controller = ZeroneController(sys.path)
        ...
        self.library_manager = LibraryManager(self.controller.config_dir)
        ...
        self.main_path = self.controller.zeroneai_settings.main_path
        
        # Set window default size

        self.set_default_size(self.settings.get_int("window-width"), self.settings.get_int("window-height"))
        # Set zoom
        self.set_zoom(self.controller.zeroneai_settings.zoom)
        # Update the settings
        self.first_load = True
        ...
        menu.append(_("Settings"), "win.open_settings")
        ...
        # Mute TTS Button removed — stop is handled by message TTS button
        # Flap button
        ...
        sidebar_header.append(logo_label)
        sidebar_header.append(menu_button)
        self.chats_secondary_box.append(sidebar_header)
        self.chats_secondary_box.append(Gtk.Separator())

        # ── Fixed nav items (New Chat + Search + Library) ─────────────────
        nav_box = Gtk.Box(
        ...
        # Row: New Chat + Search icon
        new_chat_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)

        new_chat_button = Gtk.Button(hexpand=True, css_classes=["flat"])
        new_chat_btn_inner = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8,
                                     margin_start=4, margin_end=4,
                                     margin_top=4, margin_bottom=4)
        nc_icon = Gtk.Image(icon_name="list-add-symbolic")
        nc_label = Gtk.Label(label=_("New Chat"), xalign=0, hexpand=True)
        new_chat_btn_inner.append(nc_icon)
        new_chat_btn_inner.append(nc_label)
        new_chat_button.set_child(new_chat_btn_inner)
        new_chat_button.connect("clicked", self.new_chat)
        new_chat_row.append(new_chat_button)

        self._search_toggle_btn = Gtk.ToggleButton(css_classes=["flat"],
                                                    icon_name="system-search-symbolic")
        self._search_toggle_btn.connect("toggled", self._on_sidebar_search_toggled)
        new_chat_row.append(self._search_toggle_btn)
        nav_box.append(new_chat_row)

        # Search bar (hidden by default)
        self._sidebar_search_bar = Gtk.SearchBar(show_close_button=False)
        self._sidebar_search_entry = Gtk.SearchEntry(
            placeholder_text=_("Search chats…"), hexpand=True)
        self._sidebar_search_entry.connect("search-changed", self._on_sidebar_search_changed)
        self._sidebar_search_bar.set_child(self._sidebar_search_entry)
        self._sidebar_search_bar.connect_entry(self._sidebar_search_entry)
        nav_box.append(self._sidebar_search_bar)

        # Library button
        library_btn = Gtk.Button(css_classes=["flat"])
        lib_inner = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8,
                            margin_start=4, margin_end=4,
                            margin_top=4, margin_bottom=4)
        lib_icon = Gtk.Image(icon_name="user-bookmarks-symbolic")
        lib_label = Gtk.Label(label=_("My Library"), xalign=0, hexpand=True)
        lib_inner.append(lib_icon)
        lib_inner.append(lib_label)
        library_btn.set_child(lib_inner)
        library_btn.connect("clicked", self.open_library)
        nav_box.append(library_btn)

        # TTS button
        tts_nav_btn = Gtk.Button(css_classes=["flat"])
        tts_inner = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8,
                            margin_start=4, margin_end=4,
                            margin_top=4, margin_bottom=4)
        tts_inner.append(Gtk.Image(icon_name="audio-speakers-symbolic"))
        tts_inner.append(Gtk.Label(label=_("Text to Speech"), xalign=0, hexpand=True))
        tts_nav_btn.set_child(tts_inner)
        tts_nav_btn.connect("clicked", self.open_tts_page)
        nav_box.append(tts_nav_btn)

        self.chats_secondary_box.append(nav_box)
        self.chats_secondary_box.append(Gtk.Separator())

        # ── "Recent chats" section label ──────────────────────────────────
        recent_label = Gtk.Label(
            label=_("Recent Chats"),
            css_classes=["caption", "dim-label"],
            xalign=0,
            margin_start=14, margin_top=8, margin_bottom=4,
        )
        self.chats_secondary_box.append(recent_label)
        ...
        # Keep chat_panel_header reference for compatibility
        ...
        # Wrap in NavigationView so settings/library can open as full pages
        self.nav_view = Adw.NavigationView()
        self.main_nav_page = Adw.NavigationPage(
            child=self.main_program_block,
            title=_("ZeroneAI"),
            tag="main"
        )
        ...
        # Register window-level actions for inline pages
        action_settings = Gio.SimpleAction.new("open_settings", None)
        action_settings.connect("activate", self.open_settings_page)
    ...
    def set_zoom(self, zoom):
        ...
        if settings is not None:
            ...
            self.controller.zeroneai_settings.zoom = zoom
    ...
    def update_toggles(self, *_):
        ...
        self.tts_enabled = self.controller.zeroneai_settings.tts_enabled
        self.rag_on = self.controller.zeroneai_settings.rag_on
        self.memory_on = self.controller.zeroneai_settings.memory_on
        self.virtualization = self.controller.zeroneai_settings.virtualization
    ...
    def quick_settings_update(self):
        ...
        self.tts_enabled = self.controller.zeroneai_settings.tts_enabled
        self.rag_on = self.controller.zeroneai_settings.rag_on
        self.rag_on_documents = self.controller.zeroneai_settings.rag_on_documents
        self.memory_on = self.controller.zeroneai_settings.memory_on
    ...
    def update_settings(self):
        ...
        if self.first_load:
            ...
            send_on_enter = not self.controller.zeroneai_settings.send_on_enter
            n_pages = self.chat_tabs.get_n_pages()
            for i in range(n_pages):
                page = self.chat_tabs.get_nth_page(i)
                child = page.get_child()
        ...
        self.current_profile = self.controller.zeroneai_settings.current_profile
        self.profile_settings = self.controller.zeroneai_settings.profile_settings
        self.memory_on = self.controller.zeroneai_settings.memory_on
        self.rag_on = self.controller.zeroneai_settings.rag_on
        self.tts_enabled = self.controller.zeroneai_settings.tts_enabled
        self.virtualization = self.controller.zeroneai_settings.virtualization
        self.prompts = self.controller.zeroneai_settings.prompts
        self.translation_enabled = self.controller.zeroneai_settings.translation_enabled
        # Handlers
    ...
    def unload_avatar(self, handler=None):
        pass
    ...
    def open_tts_page(self, *args):
        """Open the Text-to-Speech page."""
        existing = self.nav_view.find_page("tts")
        if existing is not None:
            self.nav_view.pop_to_tag("main")
            return
        tts_widget = TTSPage(
            controller=self.controller,
            on_close=lambda: self.nav_view.pop_to_tag("main"),
        )
    ...
    def open_library(self, *args):
        """Open the Library as a full inline page"""
        existing = self.nav_view.find_page("library")
        if existing is not None:
            self.nav_view.pop_to_tag("main")
            return

        lib_widget = LibraryWindow(
            manager=self.library_manager,
            on_use_template=self._on_library_use_template,
            on_use_character=self._on_library_use_character,
            on_close=lambda: self.nav_view.pop_to_tag("main"),
        )
        page = Adw.NavigationPage(
            child=lib_widget,
    ...
    def open_settings_page(self, *args):
        """Open Settings as a full inline page"""
        existing = self.nav_view.find_page("settings")
        if existing is not None:
            self.nav_view.pop_to_tag("main")
            return

        def on_close():
            self._on_settings_page_closed()
            self.nav_view.pop_to_tag("main")
    ...
    def _on_settings_page_closed(self):
        """Called when settings page is closed"""
        settings = Gio.Settings.new("com.zeroneai.app")
        settings.set_int("chat", self.chat_id)
        settings.set_string("path", os.path.normpath(self.main_path))
        self.update_settings()

    def _on_library_use_template(self, text: str):
        """Insert template text into the current input"""
        tab = self.get_active_chat_tab()
        if tab is not None:
            tab.set_input_text(text)
        self.notification_block.add_toast(
            Adw.Toast(title=_("Template applied"))
        )

    def _on_library_use_character(self, character):
        """Update active character system prompt"""
        if character is not None:
            self.notification_block.add_toast(
                Adw.Toast(title=_("Character activated: ") + character.name)
            )
        else:
            self.notification_block.add_toast(
                Adw.Toast(title=_("Character deactivated"))
            )

    def get_active_character_prompt(self) -> str | None:
        """Return the active character system prompt if any"""
        char = self.library_manager.get_active_character()
        if char:
            return char.content
        return None
    ...
    def start_recording(self, button):
        ...
        if os.path.exists(path):
            os.remove(path)
        self.recording = True
        if self.controller.zeroneai_settings.automatic_stt:
        ...
        self.recorder = AudioRecorder(
            ...
            silence_duration=self.controller.zeroneai_settings.stt_silence_detection_duration,
            silence_threshold_percent=self.controller.zeroneai_settings.stt_silence_detection_threshold,
        )
    ...
    def attach_file(self, button):
        ...
        if (
            ...
            and self.controller.zeroneai_settings.rag_on_documents
        ):
    ...
    def _on_sidebar_search_toggled(self, btn):
        self._sidebar_search_bar.set_search_mode(btn.get_active())
        if btn.get_active():
            self._sidebar_search_entry.grab_focus()
        else:
            self._sidebar_search_entry.set_text("")
            self.update_history()

    def _on_sidebar_search_changed(self, entry):
        query = entry.get_text().strip().lower()
        if not query:
            # Show all rows
            list_box = getattr(self, "chats_list_box", self.chats_buttons_block)
        ...
        while child:
            row = child
            child = child.get_next_sibling()
            name = getattr(row, "chat_name", getattr(row, "chat_title", "")).lower()
            row.set_visible(query in name)
    ...
    def update_history(self):
        ...
        if self.controller.zeroneai_settings.reverse_order:
        ...
        def add_chat_recursive(index, level=0):
            ...
            chat_row.connect_signals(
                ...
                on_clear=self.clear_chat_from_sidebar,
    ...
    def clear_chat_from_sidebar(self, button):
        """Clear chat messages from sidebar button"""
        chat_index = int(button.get_name())
        if chat_index == self.chat_id:
            # Clear the active chat tab
            tab = self.get_active_chat_tab()
            if tab:
                tab.clear_chat()
        else:
            # Clear stored chat data
            self.chats[chat_index] = [self.chats[chat_index][0]] if self.chats[chat_index] else []
    ...
    def chose_chat(self, id, *a):
        ...
        # Change profile 
        if self.controller.zeroneai_settings.remember_profile and "profile" in self.chats[chat_id]:
    ...
    def generate_suggestions(self):
        ...
        suggestions = self.secondary_model.get_suggestions(
            self.controller.zeroneai_settings.prompts["get_suggestions_prompt"],
            self.offers,
    ...
    def add_browser_tab(self, action=None, param=None, url=None):
        """Add a browser tab"""
        if url is None:
            url = self.controller.zeroneai_settings.initial_browser_page
        if self.controller.zeroneai_settings.browser_session_persist:
            session = self.controller.config_dir + "/bsession.json"
        else:
            session = None
        browser = BrowserWidget(url,self.controller.zeroneai_settings.browser_search_string, session)
    ...
    def add_editor_tab(self, tabview=None, file=None):
        if file is not None:
            ...
            editor = CodeEditorWidget(scheme=self.controller.zeroneai_settings.editor_color_scheme)
    ...
    def export_chat(self, export_all=False):
        ...
        if export_all:
            ...
            default_filename = f"zeroneai_chats_export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        else:
            ...
            if export_data is None:
                ...
                return
            default_filename = f"zeroneai_chat_{self.chat_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def chat_id(self):
        ...
        return self.controller.zeroneai_settings.chat_id

    @chat_id.setter
    def chat_id(self, value):
        """Set the current chat ID in the controller's settings"""
        self.controller.zeroneai_settings.chat_id = value
